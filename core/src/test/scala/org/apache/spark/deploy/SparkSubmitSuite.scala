...
class SparkSubmitSuite
  ...
  test("resolves config paths correctly") {
    ...
    val (_, _, conf4, _) = SparkSubmit.prepareSubmitEnvironment(appArgs4, conf = Some(hadoopConf))
  ...
  test("support --py-files/spark.submit.pyFiles in non pyspark application") {
    ...
    val appArgs = new SparkSubmitArguments(args)
    val (_, _, conf, _) = SparkSubmit.prepareSubmitEnvironment(appArgs, conf = Some(hadoopConf))
    ...
    val (_, _, conf1, _) = SparkSubmit.prepareSubmitEnvironment(appArgs1, conf = Some(hadoopConf))
